...
| State | Count | Rows |
| --- | ---: | --- |
| Verified | 31 | `SHELL-01`..`SHELL-03`, `SESS-01`..`SESS-03`, `MSG-01`, `MSG-02`, `MSG-05`, `MSG-06`, `STREAM-02`, `COMP-01`..`COMP-03`, `SET-01`..`SET-14`, `PAGE-02`..`PAGE-04` |
| In progress | 13 | All remaining detailed matrix rows |
...
| PAGE-01 | Automation | triggers, monitor/follow-up style workflows, session links | `AutomationView.tsx` | V1/V2 screenshot and workflow tests | In progress | Automation now supports real project edit/PATCH, interval and one-shot schedules, Running/Paused/Current grouping, cache-synchronized list/detail updates, compact stable actions, session handoff, and explicit project/session load failures rather than false empty states. Packed actions cover create, run, edit/toggle, and delete, and the live `/app/` page was inspected against the real backend. Evidence: `v2-module-actions.spec.ts`, `AutomationView.test.tsx`, `.tmp/frontend-v2-ts-module-actions/`, lint/build, and independent review of the error-state fix. Still needs formal V1 pairing, monitor/follow-up workflow inventory, and narrow loading/error closure before `Verified`. |
...
| PAGE-05 | Connectors | connector list, runtime tools, enable/status/detail flows | `ConnectorsView.tsx`, `RuntimeToolsSection.tsx` | V1/V2 screenshot and tests | In progress | Connectors now includes the real W3 GET/PUT editor, treats HTTP-200 `ok:false` as an error without closing the form, and routes provider configuration explicitly through AppShell into the existing System secondary page. List/detail/probe, runtime copy/PATH, loading, retryable error, and 720px fixed-frame states are covered; live `/app/` inspection opened GitHub directly on the GitHub secondary Settings page. Evidence: `v2-connectors-view.spec.ts` 2/2, `v2-module-actions.spec.ts` 4/4, `ConnectorsView.test.tsx`, `.tmp/frontend-v2-ts-connectors/`, lint/build, and independent reviewer PASS for the rejected-save and routing regressions. Still needs a formal complete V1/V2 page pair and remaining provider configuration inventory before `Verified`. |
| PAGE-06 | Memory | memory facts, sessions, edit/delete if V1 has it | `MemoryView.tsx` | V1/V2 screenshot and tests | In progress | Memory now has same-fixture V1/V2 desktop comparison for workspace count, selected memory/detail, Architecture, and Skill Draft list/detail content, plus a 720px frame capture. Existing coverage retains search, filters, rebuild, Architecture lifecycle, and Skill Draft generate/save/validate/apply workflows. Evidence: `.tmp/frontend-v1-v2-primary-surfaces/primary-pair-v1-memory.png`, `primary-pair-v2-memory.png`, `primary-narrow-v2-memory.png`, `.tmp/frontend-v2-ts-memory/`, focused browser/component/API tests. It remains `In progress` because secondary-page loading/error states and V1 edit/delete behavior, if exposed, still require explicit closure. |
| PAGE-07 | Observability | metrics, events, trends, lineage panels | `ObservabilityPanel.tsx`, `ObservabilityTrends.tsx`, `SpecLineagePanel.tsx` | V1/V2 screenshot and tests | In progress | Observability restores the visible V1 KPI inventory for cached/uncached input, cache ratio, retrieval count/failure/latency/docs, skill calls, MCP calls, and Gateway metrics. Missing values render as `—`; Gateway sections render only when gateway-specific data exists. Packed coverage now includes skeleton loading and 720px framing in addition to scope, trends, breakdowns, lineage, empty, and error states, and the real `/app/` surface was visually inspected. Evidence: `v2-observability.spec.ts` 3/3, `ObservabilityPanel.test.tsx`, `.tmp/frontend-v2-ts-observability/`, lint/build, real-browser inspection, and independent reviewer PASS for absent values/Gateway gating. It remains `In progress` pending formal V1 pairing and V1 event-specific inventory closure. |
