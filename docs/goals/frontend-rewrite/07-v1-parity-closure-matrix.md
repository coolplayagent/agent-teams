...
| MSG-01 | Historical replay | round marker, prompt, processed work, final answer, copy placement | `MessageTimeline.tsx`, `RoundMarker.tsx` | Complex history replay screenshots and DOM row snapshots | In progress | Expanded round markers now use native details toggling with only the active action label in the summary and the full prompt once in the body; real browser DOM check on a long Chinese prompt confirmed `summaryText="收起"`, `titleInSummary=false`, and one prompt-prefix occurrence after rebuild. Evidence: `.tmp/round-marker-expanded-dom.json`, `.tmp/round-marker-expanded.png`. Still needs complex hard-refresh replay comparison and divider/copy placement review. |
...
| --- | --- | --- |
| MSG-01 | Prompt appears only in the round marker, not duplicated as a user row or expanded summary/body pair. | Added expanded prompt browser fixture, controlled `RoundMarker` unit coverage, and a real browser DOM/screenshot check for the long Chinese prompt expansion. Still must hard-refresh broader complex histories in browser. |
