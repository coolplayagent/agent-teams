...
| --- | --- | --- |
| MSG-01 | Prompt appears only in the round marker, not duplicated as a user row or expanded summary/body pair. | Added expanded prompt browser fixture, controlled `RoundMarker` coverage, and a real browser DOM/screenshot check proving the open marker has no visible title duplicate. Still must hard-refresh broader complex histories in browser. |
