...
| STREAM-02 | Replay and reconnect | `after_event_id`, Last-Event-ID fallback, duplicate events, network reconnect, terminal settlement | `streamClient.ts`, `useRunStreamController.ts` | Tests and browser interrupted-stream scenario | In progress | Session-switch browser spec checks ordered switch-back recovery for text deltas; subagent SSE carries scoped runtime state so UUID child rows do not leak into the parent timeline; persisted child messages are filtered before round metadata hydrates; and tool lifecycle coverage proves a hard-refreshed pending tool call reconnects from `after_event_id=2`, receives event 3 as the result, and renders one completed card without losing repeated post-tool text. Managed backend coverage now verifies active hard refresh and real TCP interruption across normal text, normal tool lifecycle, orchestration tool lifecycle, and right-panel subagent streams. The network probe requires a failed SSE connection, a second request carrying positive `after_event_id` or `Last-Event-ID`, post-reconnect growth, and terminal settlement without duplicate prefix, repeated tail, stale cursor, missing answer, role leakage, or parent/child row leakage. The subagent endpoint now emits standard SSE `id:` lines, honors the newer query/header cursor, and the panel keeps a bounded manual cursor fallback. Evidence: `npm run test -- src/test/RunStreamController.test.tsx src/test/SubagentSessionView.test.tsx`, `npm run test:browser -- v2-tool-lifecycle.spec.ts --project=chromium`, `npm run test:browser -- v2-stream-refresh.spec.ts --project=chromium -g "tool-heavy"`, `npm run test:browser -- v2-stream-reconnect.spec.ts --project=chromium -g "non-text"`, `AGENT_TEAMS_MANAGED_LIVE_STREAM=1 npm run test:browser -- v2-managed-backend-live-stream.spec.ts --project=chromium`, `AGENT_TEAMS_REAL_LIVE_STREAM=1 npm run test:browser -- v2-real-backend-live-stream.spec.ts --project=chromium`. Still must prove real-provider interruption/reconnect and complete final paired V1/V2 visual sign-off. |
| REC-01 | Recovery bar | active run, stopped run, resume, local reconciliation | `RecoveryBar.tsx` | Browser refresh during active run and stopped run | In progress | Browser recovery coverage now includes an active-run refresh path that opens from `last_event_id=17`, streams event 18, reloads with event 18 persisted, reconnects from `last_event_id=18`, and proves the persisted event is not duplicated before event 19 arrives. Existing mock and real-SSE coverage also proves standalone stopped-run resume uses the checkpoint and real-SSE active stop suppresses stale reconnect. Managed real-backend coverage now adds active hard-refresh continuation, stale selected-session reconciliation, and actual TCP interruption/reconnect for normal, tool-heavy, orchestration, and subagent streams. The controller keeps interrupted streams recoverable from the latest local/recovery cursor; subagent native reconnect now has SSE IDs plus a bounded manual fallback. Removed the obsolete `tests/unit_tests/frontend/test_recovery_stream_ui.py` static V1/dist harness after current V2 RecoveryBar and browser recovery coverage took ownership. Evidence: `npm run test -- src/test/RunStreamController.test.tsx src/test/SubagentSessionView.test.tsx`, `npm run test:browser -- v2-recovery.spec.ts --project=chromium`, `npm run test:browser -- v2-real-sse-stale-recovery.spec.ts -g "reopens a real SSE stream from the recovery checkpoint after refresh|resumes a real SSE recoverable run from the standalone action|stops a real SSE active run" --project=chromium`, `AGENT_TEAMS_MANAGED_LIVE_STREAM=1 npm run test:browser -- v2-managed-backend-live-stream.spec.ts --project=chromium`, screenshots `.tmp/frontend-v2-ts-recovery/v2-active-recovery-refresh.png`, `.tmp/frontend-v2-ts-stream/v2-real-sse-refresh-recovery.png`, `.tmp/frontend-v2-ts-stream/v2-real-sse-recoverable-resume.png`, `.tmp/frontend-v2-managed-backend-live/managed-live-subagent-panel-after-network-recovery.png`. Still needs true real-provider interruption evidence and broader final manual/V1 comparison before `Verified`. |
...
| PAGE-02 | Skills | skill list, install/status/detail flows | `SkillsView.tsx` | V1/V2 screenshot and interaction tests | In progress | Skills now has dedicated packed `/app/` browser coverage for primary-nav entry, ClawHub market browse/search, market loading skeleton, market pagination beyond one page, market detail drawer, detail load failure, install endpoint payload, install failure toast, installed-skill list, installed load failure, installed detail drawer, user skill uninstall confirmation, skills reload, ClawHub token probe/save, ClawHub probe failure feedback, narrow-width layout overflow checks, fixed-shell framing, and inspected screenshots. Component coverage remains active for market browse/search/install/load-more, quick install drawer, ClawHub settings/probe/save, installed detail markdown, saved-token handling, and duplicate installed skill names remaining distinguishable by ref/source without leaking raw source identifiers. Evidence: `npm run test:browser -- v2-skills-view.spec.ts --project=chromium`, `npm test -- src/test/SkillsView.test.tsx`, `npm test -- src/test/SkillsView.test.tsx -t "duplicate installed skill"`, `npm run lint`, screenshots `.tmp/frontend-v2-ts-skills/v2-skills-market.png`, `.tmp/frontend-v2-ts-skills/v2-skills-market-loading.png`, `.tmp/frontend-v2-ts-skills/v2-skills-market-paged.png`, `.tmp/frontend-v2-ts-skills/v2-skills-detail-error.png`, `.tmp/frontend-v2-ts-skills/v2-skills-installed-error.png`, `.tmp/frontend-v2-ts-skills/v2-skills-installed-detail.png`, `.tmp/frontend-v2-ts-skills/v2-skills-clawhub-settings.png`, `.tmp/frontend-v2-ts-skills/v2-skills-clawhub-probe-failed-narrow.png`. Still needs formal V1 screenshot/DOM pairing and reviewer sign-off before `Verified`. |
| PAGE-03 | Board | board/todo source groups, statuses, sync, filters | `BoardTodosView.tsx` | V1/V2 screenshot and tests | In progress | Board now has browser coverage for the real `/app/` surface: sidebar navigation, workspace-scoped board load, source summary count, status columns, search filtering without stale cards, archived-column toggle with `include_archived=true`, sync replacement, handoff preview/start, request-changes preview/start, board source edit/create/delete, loading skeleton, load failure alert, and narrow-width layout checks through real mocked endpoints. Evidence: `npm run test:browser -- v2-board-actions.spec.ts --project=chromium`, `npm run test -- src/test/BoardTodosView.test.tsx`, screenshots `.tmp/frontend-v2-ts-board-actions/v2-board-filtered-review.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-archived-visible.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-sync.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-handoff-started.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-request-changes.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-source-settings.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-loading-narrow.png`, `.tmp/frontend-v2-ts-board-actions/v2-board-load-error-narrow.png`. Still needs formal V1 screenshot/DOM pairing and reviewer sign-off before `Verified`. |
| PAGE-04 | Search | session search, result selection, highlighting, empty/error states | `SessionSearchView.tsx` | Browser workflow and tests | In progress | Search now has a browser path on `/app/` covering sidebar navigation into Search, multi-workspace session filtering, query highlighting, empty results, keyboard `Enter` selection, return to Chat, selected-session sidebar state, target-message hydration, loading skeleton state, and fixed-shell scroll/composer checks. Focused component tests cover title/workspace filtering, click selection, keyboard selection, empty state, and load-error state. Evidence: `npm run test:browser -- v2-search-view.spec.ts --project=chromium`, `npm run test -- src/test/SessionSearchView.test.tsx`, screenshots `.tmp/frontend-v2-ts-search/v2-session-search-loading.png`, `.tmp/frontend-v2-ts-search/v2-session-search-filtered-results.png`, `.tmp/frontend-v2-ts-search/v2-session-search-selected-chat.png`. Still needs formal V1 screenshot/DOM pairing and reviewer sign-off before `Verified`. |
