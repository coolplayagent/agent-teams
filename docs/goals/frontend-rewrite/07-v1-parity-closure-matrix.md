...
| MSG-05 | Processed group | terminal work folds under one `已处理` affordance, no decorative divider abuse | `MessageTimeline.tsx`, CSS | Screenshot, click expand/collapse, virtual row measurement test | In progress | Browser coverage now verifies a completed run renders one collapsed `Processed` control without the old `.at-processed-group-line` divider, keeps thinking/tool work hidden while the final answer remains visible, expands on click, and increases both processed-row height and virtual timeline height after remeasurement. Evidence: `npm run test:browser -- v2-processed-group.spec.ts --project=chromium`, screenshot `.tmp/frontend-v2-ts-processed-group/v2-processed-group-expanded.png`. Still needs broader complex-history click screenshots before `Verified`. |
...
| MSG-05 | `已处理` is a real compact fold control, not divider decoration, and preserves virtualizer measurements. | Unit coverage exercises processed group folding in hydrated thinking/tool flows; `v2-processed-group.spec.ts` now proves collapsed/expanded browser behavior, absence of old divider DOM, and virtual height remeasurement. Still needs broader complex-history click screenshots. |
...
| SUB-01 | Right-side subagent panel remains readable and resizable across replay/live panel states. | `AppShell.test.tsx` now clamps panel growth to the measured workspace width; `MessageTimeline.test.tsx` now covers scoped subagent events carrying the primary role and subagent rounds injected from replay. `SubagentSessionView.test.tsx` now covers terminal state reaching the panel before history refill finishes while preserving live runtime output. Browser verification showed main `.at-chat-view` has no detached Crafter/subagent rows while the right panel keeps the child rows/output and no terminal spinner. Still needs hard-refresh replay and broader orchestration variants. |
