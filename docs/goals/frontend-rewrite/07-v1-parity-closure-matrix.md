...
| State | Count | Rows |
| --- | ---: | --- |
| Verified | 6 | `SET-01`..`SET-05`, `SET-14` |
| In progress | 38 | All remaining matrix rows |
...
Primary evidence: `npm run test:browser -- v2-settings-parity.spec.ts
v2-appearance-layout.spec.ts --project=chromium` (12/12), the complete
`SettingsDrawer.test.tsx` suite (62/62), focused shared/navigation tests (5/5),
`npm run lint`, `npm run build`, the paired JSON above, independent reviewer
PASS, and inspected screenshots under `.tmp/frontend-v2-ts-settings-parity/`.
