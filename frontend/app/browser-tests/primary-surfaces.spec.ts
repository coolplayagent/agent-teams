...
const SCREENSHOT_FOLDER = "frontend-primary-surfaces";
...
test("verifies primary surfaces for Skills, Board, Search, and Memory", async ({
  page,
}) => {
...
test("keeps primary surfaces horizontally framed at 720x760", async ({
  page,
}) => {
  const appServer = await serveFrontendDist();
  const unhandledApiRoutes: string[] = [];
