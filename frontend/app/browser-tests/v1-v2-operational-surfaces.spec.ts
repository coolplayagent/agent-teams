...
const SCREENSHOT_FOLDER = "frontend-v1-v2-operational-surfaces";

test("pairs V1 and V2 Automation from one operational fixture", async ({ page }) => {
  await runOperationalPair(page, assertV1Automation, assertV2Automation);
});

test("pairs V1 and V2 Connectors from one operational fixture", async ({ page }) => {
  await runOperationalPair(page, assertV1Connectors, assertV2Connectors);
});

test("pairs V1 and V2 Observability from one operational fixture", async ({ page }) => {
  await runOperationalPair(page, assertV1Observability, assertV2Observability);
});
...
async function runOperationalPair(
  page: Page,
  assertV1: (page: Page) => Promise<void>,
  assertV2: (page: Page) => Promise<void>,
): Promise<void> {
  const appServer = await serveFrontendDist();
  const unhandledApiRoutes: string[] = [];
  try {
    ...
    await expectNoDocumentScroll(
      page,
      "paired operational surfaces should stay inside their fixed app frames",
    );
  } finally {
    await appServer.close();
  }
