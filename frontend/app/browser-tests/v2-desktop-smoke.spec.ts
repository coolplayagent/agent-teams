...
const desktopArtifactRoot = join(repoRoot, ".tmp", "frontend-v2-desktop");
...
test("electron loads renderer with isolated preload", async () => {
  ...
  try {
    ...
    try {
      ...
      await page.waitForURL(`${backend.url}/app/`, { timeout: waitTimeoutMs });
      ...
      try {
        ...
        await saveDesktopScreenshot(page, "v2-electron-renderer-failed.png");
      ...
      await saveDesktopScreenshot(page, "v2-electron-renderer.png");
    } finally {
...
test("electron shows backend startup failure", async () => {
  ...
  try {
    ...
    try {
      ...
      await saveDesktopScreenshot(page, "v2-electron-startup-failed.png");
    } finally {
      await browser.close().catch(() => undefined);
    }
  } finally {
  ...
  try {
    ...
    try {
      ...
      await expect
        ...
        .toContain("https://example.com/docs?source=markdown#v2");
      ...
      await expect
        .poll(() => readTextIfExists(externalLog))
        .toContain("https://example.com/docs?source=electron#v2");
...
test("electron managed backend starts and stops with main lifecycle", async () => {
  ...
  try {
    ...
    expect(requestLogText).toContain("/app/");
...
function apiResponse(path: string): JsonResponse {
  ...
  if (path === `/api/sessions/${sessionId}/messages`) {
    const text = "Electron renderer smoke [Desktop docs](https://example.com/docs?source=markdown#v2)";
    return [
      {
        content: text,
...
function resolveDesktopFile(requestPath: string): string | null {
  let target = join(appRoot, "index.html");
  if (requestPath.startsWith("/app/") && requestPath !== "/app/") {
    target = join(appRoot, requestPath.slice("/app/".length));
  } else if (requestPath !== "/app" && requestPath !== "/app/") {
    const candidate = join(frontendRoot, requestPath.slice(1));
    target = existsSync(candidate) ? candidate : join(frontendRoot, "index.html");
  }
  const resolvedTarget = resolve(target);
  const allowedRoots = [resolve(appRoot), resolve(frontendRoot)];
  return allowedRoots.some((root) => resolvedTarget.startsWith(root))
    ? resolvedTarget
    : null;
}
