...
class RunInjectionManager:
    def __init__(self) -> None:
        ...
        self._queues: dict[str, list[InjectionMessage]] = {}
    ...
    def enqueue(self, run_id: str, source: InjectionSource, content: str) -> InjectionMessage:
        priority = _priority_for(source)
        message = InjectionMessage(run_id=run_id, source=source, content=content, priority=priority)
        with self._lock:
            if run_id not in self._active_runs:
                raise KeyError(f'Run is not active: {run_id}')
            self._queues.setdefault(run_id, []).append(message)
        return message

    def drain_at_boundary(self, run_id: str) -> tuple[InjectionMessage, ...]:
        with self._lock:
